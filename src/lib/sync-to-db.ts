            ...
            await Promise.all(emails.map(email => {
                return limit(async () => {
                    ...
                    await oramaClient.insert({
                        ...
                        to: email.to.map((t: { name: any; address: any; }) => `${t.name} <${t.address}>`),
                        sentAt: new Date(email.sentAt).toLocaleString(),
...
async function upsertEmail(email: EmailMessage, index: number, accountId: string) {
    ...
    try {
        ...
        const toAddresses = email.to.map((addr: { address: string; }) => addressMap.get(addr.address)).filter(Boolean);
        const ccAddresses = email.cc.map((addr: { address: string; }) => addressMap.get(addr.address)).filter(Boolean);
        const bccAddresses = email.bcc.map((addr: { address: string; }) => addressMap.get(addr.address)).filter(Boolean);
        const replyToAddresses = email.replyTo.map((addr: { address: string; }) => addressMap.get(addr.address)).filter(Boolean);
